...
gsapWithCSS.registerPlugin(ScrollTrigger, ScrollSmoother, Flip, SplitText);
  ...
  async function preloader() {
    ...
    mm.add(
      ...
      (context) => {
        ...
        if (isLarge) {
          let tlLetter = function(letters, value = 0, direction = true, dur = 0.5, stag = 0.02) {
          ...
          }, counter = function(dur = 3.5) {
            gsapWithCSS.to(counterElement, {
              duration: dur,
              innerHTML: 100,
          ...
          }, heroImagesAnimation = function() {
            let images2 = document.querySelectorAll(
              '[data-animation="hero-images-review"]'
            );
            gsapWithCSS.set(images2, {
              yPercent: 100
            });
            let tl = gsapWithCSS.timeline();
            tl.to(images2, { autoAlpha: 1, duration: 0.01 }).to(images2, {
              yPercent: 0,
              stagger: 0.2,
              duration: 0.9,
              ease: "power1.out"
            ...
            let tl = gsapWithCSS.timeline({
              onComplete: () => {
                ...
                gsapWithCSS.set(".footer", { autoAlpha: 1, duration: 0.01 });
                splits.title.revert();
              }
            });
            gsapWithCSS.set(".hero__title", { autoAlpha: 1, duration: 0.01 });
            tl.to(splits == null ? void 0 : splits.title.lines[0].querySelectorAll(".char"), {
              yPercent: 0,
              stagger: 0.02,
              // Задержка между символами
              duration: 0.5,
              // Длительность анимации для первой строки
              ease: "power1.out"
            }).to(
              splits == null ? void 0 : splits.title.lines[1].querySelectorAll(".char"),
              {
                yPercent: 0,
                stagger: 0.02,
                // Можно задать другой stagger для разнообразия
                duration: 0.5,
                // Можно задать другую длительность
                ease: "power1.out"
              },
              "<"
            ...
            return tl;
          }, navLinksAnimation = function() {
            let charsDuration = 0.25;
            let charsStagger = 5e-3;
            let overlapAmount = 0.2;
            gsapWithCSS.set(navLinks, { autoAlpha: 1, duration: 0.01 });
            let navTl = gsapWithCSS.timeline();
            navLinks.forEach((link, index) => {
              let chars = gsapWithCSS.utils.toArray(".char", link);
            ...
            let charsDuration = 0.25;
            let charsStagger = 9e-3;
            let charsSwitcher = gsapWithCSS.utils.toArray(".char", switcher);
            gsapWithCSS.set(switcher, { autoAlpha: 1, duration: 0.01 });
            let tl = gsapWithCSS.timeline({
              onComplete: () => {
            ...
            return tl;
          }, heroBtnAnimation = function() {
            let charsDuration = 0.25;
            let charsStagger = 9e-3;
            gsapWithCSS.set(btn, { autoAlpha: 1, duration: 0.01 });
            return gsapWithCSS.to(charsBtn, {
              yPercent: 0,
              stagger: charsStagger,
              duration: charsDuration
            });
          }, logoTextAnimation = function() {
            let charsDuration = 0.25;
            let charsStagger = 9e-3;
            gsapWithCSS.set(logoText, { autoAlpha: 1, duration: 0.01 });
            return gsapWithCSS.to(logoTextChars, {
              yPercent: 0,
              stagger: charsStagger,
              duration: charsDuration
            });
          ...
          let navLinks = gsapWithCSS.utils.toArray(".header__nav-link");
          ...
          let charsBtn = gsapWithCSS.utils.toArray(".char", btn);
          let logoText = document.querySelector(".logo-loader__text ");
          let logoTextChars = gsapWithCSS.utils.toArray(".char", logoText);
          let valueFrom = 102;
          let valueTo = -102;
          ...
          gsapWithCSS.set([loader, counterWrapper], { autoAlpha: 1, duration: 0.01 });
          gsapWithCSS.set(letterFigure, { yPercent: valueFrom });
          gsapWithCSS.set(splits == null ? void 0 : splits.title.chars, { yPercent: yMovePercent });
          gsapWithCSS.set(splits == null ? void 0 : splits.header.chars, { yPercent: yMovePercent });
          ...
          tlMaster.add("start", "0").to([letterM, letterA, letterX], { yPercent: 0, stagger: 0.02 }).to(letterMove, { yPercent: 0 }, "start").add(tlLetter(mikhailovLetters, 0), "<").add(tlLetter(architectureLetters), "start").to(counterWrapper, { yPercent: 0 }, "start").call(counter, null, "start+=0.5").add("timerEnd", "4").to(counterWrapper, { yPercent: valueTo }, "timerEnd").to(letterX, { yPercent: valueTo, duration: 0.4 }, "timerEnd").to(letterA, { yPercent: valueTo, duration: 0.4 }, "-=98%").to(
            letterFigure,
            { yPercent: 0, duration: 0.6, ease: "power2.out" },
            "<+=20%"
          ).add(tlLetter(mikhailovLetters, valueTo, false), "<").add(tlLetter(architectureLetters, valueTo, false), "<+=5%").call(flip, null, "<").add("flipStart", "<").add(heroImagesAnimation, "flipStart+=0.5").add(titleAnimation).add(logoTextAnimation, "<").add(navLinksAnimation).add(switchAnimation, "<+=0.2").add(heroBtnAnimation, "<+=0.2");
        }
        if (isSmall) {
          let logoLetterAnimation = function() {
            let tl = gsapWithCSS.timeline();
            tl.to(logoLetter[0], {
              yPercent: 0,
              duration: 0.5,
          ...
          let logoLetter = gsapWithCSS.utils.toArray(
    ...
    mm.add(
      ...
      (context) => {
        ...
        if (isLarge) {
          ...
          let tlMoveImage = gsapWithCSS.timeline({
            scrollTrigger: {
              trigger: ".hero",
              id: "image",
              start: "top top",
          ...
          let logoText = document.querySelector(".logo-loader__text");
          let tlLogoScale = gsapWithCSS.timeline({
            scrollTrigger: {
              trigger: ".hero",
              start: "top top",
              end: "40%",
          ...
          tlLogoScale.to(logo, { scale: 0.4, duration: 1, transformOrigin: "top left" }).to(logoText, { autoAlpha: 0, yPercent: -300 }, "<");
          gsapWithCSS.timeline({
            onComplete: () => {
  ...
  function teamGridAnimation() {
    ...
    function enableAnimation() {
      handleMouseEnter = (e) => {
        ...
        miniatures.forEach(
          (item) => item.classList.remove("team__item--active")
        );
        showBlocks.forEach(
          (block) => block.classList.remove("team__show--active")
        );
